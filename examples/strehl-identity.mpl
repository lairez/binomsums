...
S1 := Sum(z^n*Sum(Binomial(n,k)*Multinomial([n,k])*Sum(Binomial(k,j)^3, j=0..infinity), k=0..infinity), n=0..infinity);
S2 := Sum(z^n*Sum(Binomial(n,k)^2*Binomial(n+k,k)^2, k=0..infinity), n=0..infinity);
